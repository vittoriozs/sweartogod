              ...
              <div className="flex items-center relative group cursor-pointer">
                {blog?.blogcategories?.map(
                  (item: { title: string }, index: number) => (
                    <p
                      key={index}
                      className="ont-semibold text-black tracking-wider 
...
const BlogLeft = async ({ slug }: { slug: string }) => {
  ...
  return (
    <div>
      <div className="border border-lighter-grey p-5 rounded-md">
        <Title className="text-base">Blog Categories</Title>
        <div className="space-y-2 mt-2">
          {categories?.map(({ blogcategories }, index) => (
            <div
              key={index}
              className="text-black/80 flex items-center justify-between text-sm font-medium"
            >
      ...
      <div className="border border-lighter-grey p-5 rounded-md mt-10">
            ...
            >
              {blog?.mainImage && (
                <Image
                  ...
                  width={100}
                  height={100}
                  className="w-16 h-16 rounded-full object-cover border border-light-grey group-hover:border-black hoverEffect"
                />
              )}
              <p className="line-clamp-2 text-sm text-black/80 group-hover:text-black hoverEffect">
                {blog?.title}
              </p>
            </Link>
          ))}
